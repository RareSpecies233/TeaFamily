export async function mount(container, ctx) {
  ...
  container.innerHTML = `
    <section style="font-family: Manrope, 'Noto Sans SC', sans-serif; border:1px solid #f0dcc0; border-radius:14px; padding:16px; background:linear-gradient(135deg,#fff8ea,#eefaff)">
      <h3 style="margin:0 0 6px; color:#2f220f;">文件管理插件</h3>
      <p style="margin:0 0 12px; color:#7e6a49;">用于安装、启动、停止与卸载 HoneyTea/LemonTea 的文件管理子进程。</p>
      <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center; margin-bottom:12px;">
        <select id="fm-node" style="padding:8px 10px; border-radius:8px; border:1px solid #dbc9aa; min-width:220px;"></select>
        <button id="fm-refresh" style="padding:8px 12px; border:0; border-radius:8px; background:#d9ece4; cursor:pointer;">刷新节点</button>
        <button id="fm-start-local" style="padding:8px 12px; border:0; border-radius:8px; background:#2f8f6d; color:#fff; cursor:pointer;">启动 LemonTea 侧</button>
        <button id="fm-stop-local" style="padding:8px 12px; border:0; border-radius:8px; background:#d08b22; color:#fff; cursor:pointer;">停止 LemonTea 侧</button>
        <button id="fm-start-remote" style="padding:8px 12px; border:0; border-radius:8px; background:#3e7bb8; color:#fff; cursor:pointer;">启动 HoneyTea 侧</button>
        <button id="fm-stop-remote" style="padding:8px 12px; border:0; border-radius:8px; background:#785fa8; color:#fff; cursor:pointer;">停止 HoneyTea 侧</button>
      </div>
      <pre id="fm-log" style="margin:0; min-height:220px; max-height:340px; overflow:auto; border-radius:10px; background:#18181a; color:#d6f2ff; padding:12px; font:12px/1.5 ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace;"></pre>
    </section>
  `

  const nodeSelect = container.querySelector('#fm-node')
  const logEl = container.querySelector('#fm-log')
